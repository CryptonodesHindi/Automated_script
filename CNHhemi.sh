...
if [ "$ARCHITECTURE" == "amd64" ]; then
    ...
    wget -q https://github.com/hemilabs/heminetwork/releases/download/v0.11.5/heminetwork_v0.11.5_linux_amd64.tar.gz
elif [ "$ARCHITECTURE" == "arm64" ]; then
    echo -e "${GREEN}Downloading arm64 tar file...${NC}"
    wget -q https://github.com/hemilabs/heminetwork/releases/download/v0.11.5/heminetwork_v0.11.5_linux_arm64.tar.gz
else
...
if [ "$ARCHITECTURE" == "amd64" ]; then
    ...
    tar xvf heminetwork_v0.11.5_linux_arm64.tar.gz
fi

# Step 7: Remove the downloaded tar.gz files
echo -e "${YELLOW}Removing tar.gz files...${NC}"
rm -f heminetwork_v0.11.5_linux_*.tar.gz
